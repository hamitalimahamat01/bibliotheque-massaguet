...
export default function PrepaBEFPage() {
  ...
  const loadBooks = async () => {
    ...
    try {
      ...
      console.error('Erreur chargement livres BEF:', error);
  ...
  return (
    <div className="animate-fade-in">
      <ModuleHeader
        title="Prépa BEF"
        description="Documents pour les élèves de 3ème"
      ...
      {loading ? (
                ...
                <Link href={`/books/${book.id}`} className="block h-full">
                  <div className="relative h-48 bg-gradient-to-br from-blue-50 to-indigo-50 flex items-center justify-center">
                    {book.coverUrl ? (
                      ...
                      <Icons.Book className="w-12 h-12 text-blue-400" />
                    )}
        ...
        <div className="text-center py-16 bg-white rounded-2xl border border-gray-100">
          <div className="w-20 h-20 bg-blue-100 rounded-2xl flex items-center justify-center mx-auto mb-4">
            <Icons.Graduation className="w-10 h-10 text-blue-400" />
          </div>
          <h3 className="text-lg font-medium text-gray-600">Aucun document trouvé</h3>
          <p className="text-gray-400 mt-1">Aucun document pour la Prépa BEF</p>
          {user && (
            <Link
              href="/books/upload"
              className="inline-flex items-center gap-2 mt-4 px-6 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl hover:shadow-lg transition-all"
            >
