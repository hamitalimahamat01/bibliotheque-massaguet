...
interface Book {
  ...
  _count: { downloadsHistory: number; favorites: number; reviews: number };
}
...
export default function BookDetailPage() {
  ...
  const handleFavorite = async () => {
    ...
    try {
      await booksApi.toggleFavorite(book!.id);
  ...
  const handleReview = async (e: React.FormEvent) => {
    ...
    try {
      await booksApi.addReview(book!.id, { rating, comment: comment || undefined });
  ...
  const getFileColor = () => {
    ...
    return colors[book.fileType] || 'bg-gray-100 text-gray-600';
  };
      ...
      <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
        <div className="grid md:grid-cols-3 gap-8 p-8">
          ...
          <div className="md:col-span-1">
            <div className={`aspect-[3/4] rounded-xl flex flex-col items-center justify-center ${getFileColor()} bg-opacity-10`}>
              <span className="text-7xl mb-4">{getFileIcon()}</span>
              <span className={`text-sm font-bold px-4 py-2 rounded-full ${getFileColor()}`}>
          ...
          <div className="md:col-span-2">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm my-4">
              ...
              <div>
                ...
                <p className="font-medium">{book.downloads}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <p className="font-medium">{book._count?.favorites || 0}</p>
              </div>
              <div>
                <span className="text-gray-500">Partagé par</span>
                <p className="font-medium">{book.uploadedBy?.name}</p>
              </div>
            </div>
        ...
        <div className="border-t border-gray-100 p-8">
          ...
          {book.reviews && book.reviews.length > 0 ? (
            ...
            <p className="text-gray-500 mb-4">Aucun avis pour ce livre</p>
          )}
