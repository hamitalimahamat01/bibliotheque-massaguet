...
app.put('/api/auth/profile', auth, async function(req, res) {
  try {
    ...
    const result = await pool.query(
      'UPDATE users SET name = $1, bio = $2 WHERE id = $3 RETURNING id, name, email, role, bio, avatar, documents_count, created_at',
...
app.get('/api/books/:id/download', async function(req, res) {
  try {
    ...
    if (book.file_url && book.file_url.includes('cloudinary.com')) {
      ...
      // 🔥 Essayer avec resource_type 'raw' (pour les nouveaux documents)
      ...
      // 🔥 Si le fichier raw n'existe pas, essayer avec 'image' (pour les anciens documents)
      try {
        const checkRaw = await cloudinary.api.resource(publicId, { resource_type: 'raw' });
        console.log('✅ Fichier raw trouvé');
      } catch (error) {
        console.log('⚠️ Fichier raw non trouvé, utilisation de l\'URL image');
        // Utiliser l'URL image pour les anciens documents
